...
+ (id)loadingViewInView:(UIView *)aSuperview {
	CGRect rectangleFrame = CGRectMake(80, 60, 150, 150);
	LoadingView *loadingView = [[[LoadingView alloc] initWithFrame:rectangleFrame] autorelease];
	if (!loadingView) {
		return nil;
	}
	...
	loadingLabel.text = NSLocalizedString(@"Cargando ...", nil);
	...
	activityIndicatorRect.origin.y = loadingLabel.frame.origin.y + loadingLabel.frame.size.height + 20;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGPathRef roundRectPath = [self newPathWithRoundRect:rect withRadious:ROUND_RECT_CORNER_RADIUS];
	
	CGContextRef context = UIGraphicsGetCurrentContext();

	const CGFloat BACKGROUND_OPACITY = 0.70;
